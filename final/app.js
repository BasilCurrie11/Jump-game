...
//Root velocity is zero by default, jumping sets it to 
...
function animate() {
  ...
  //Makes root jump when you click anywhere
  window.addEventListener ('click', () =>{
    if (root.position.y <= 0){
        rootvel = 0.03
    };
  //Since I can't find a way to autoplay music, this makes the music play when you click anywhere
    aud.muted = false
  });
  //Makes root jump when you tap any key
  window.addEventListener ('keydown', () =>{
    if (root.position.y <= 0){
        rootvel = 0.03
    }
  //Also unmutes the music
  aud.muted = false
  });

//Root velocity is zero by default, jumping sets it to 0.03, and gravity lowers root velocity with every frame.
  root.position.y += rootvel;
  
//Gravity acceleration while jumping. Stops when it hits the ground
  if (root.position.y >= 0){
    rootvel -= gravity;
  }
  else{
...
//Makes root spin off to the side when you die
...
//Makes root shoot off to the side. Rootrotation is the rate of spin and ztranslation is the horizontal velocity.
  root.rotation.x += rootrotation;
  root.position.z += ztranslation;

  
//Resets root position after you die
